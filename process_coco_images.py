...
for i in tqdm(range(0,len(annotations))): #do everything relative to the annotation number
	...
	#                    rle = [s]
			...
			misc.imsave(output_images + '/image_' + str(training_image_category_id[0]) + '_' + str(i) + '_' + str(s) + '.jpg',masked_im)
...
np.savez(home_dir + '/coco_processed_labels',training_image_category_id=training_image_category_id,training_content_category_id=training_content_category_id,training_image_category_name=training_image_category_name,training_content_category_name=training_content_category_name)
